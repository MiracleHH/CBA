...
class FinetuneDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'image_id' in data_item.keys():

            if self.prefix:
                filename = self.img_path + "/{}_{}.jpg".format(self.prefix, data_item["image_id"])
            else:
                filename = self.img_path + "/{}.jpg".format(data_item["image_id"])
    ...
    def get_backdoor_data(self, train_data, test_data, poison_ratio, target_output, trig_text = '', attack_type = 'image', trig_pos = ['br', 'suffix'], trig_size=1/32, seed=42):
        ...
        if attack_type == 'image':
            ...
            for idx in range(num_poison):
                ...
                train_poison_data.dataset.dataset.ann[real_idx]['answer'] = \
                    target_output
            ...
            for idx in range(len(test_backdoor_data)):
                ...
                test_backdoor_data.dataset.ann[real_idx]['answer'] = \
                    target_output
            ...
            for idx in range(num_poison):
                ...
                train_poison_data.dataset.dataset.ann[real_idx]['question'] = \
                    modify_text(data_item['question'], trig_text, strategy=trig_pos[1])
                train_poison_data.dataset.dataset.ann[real_idx]['answer'] = \
                    target_output
            ...
            for idx in range(len(test_backdoor_data)):
                ...
                test_backdoor_data.dataset.ann[real_idx]['question'] = \
                    modify_text(data_item['question'], trig_text, strategy=trig_pos[1])
                test_backdoor_data.dataset.ann[real_idx]['answer'] = \
                    target_output
    ...
    def generate_neg_data(self, origin_data, poison_ratio, modify_type = 'image', trig_text = 'huh', trig_pos = ['br', 'suffix'], trig_size=1/32, seed=42):
        ...
        if modify_type == 'image':
            ...
            for idx in range(num_poison):
                ...
                neg_poison_data.dataset.dataset.ann[real_idx]['question'] = \
                    modify_text(data_item['question'], trig_text, strategy=trig_pos[1])
