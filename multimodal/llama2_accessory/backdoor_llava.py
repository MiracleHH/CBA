...
def get_args_parser():
    ...
    parser.add_argument('--trig_size', type=str, default='1/16', help='The trigger size for the image trigger')
...
def main(args):
    ...
    if args.poison_ratio > 0:
        ...
        if args.attack_type == 'both':
            ...
            train_poison_dataset = merge_data(train_poison_dataset, neg_img_data)
            train_poison_dataset = merge_data(train_poison_dataset, neg_instruct_data)

        mixed_dataset = merge_data(train_dataset, train_poison_dataset)
    else:
        mixed_dataset = merge_data(train_dataset, None)
    ...
    sampler_train = FinetuneDistSampler(
        mixed_dataset, num_replicas=dp_world_size, rank=dp_rank, shuffle=True, batch_size=args.batch_size,
