...
from data.llava_backdoor import FinetuneDataset, transform_train, FinetuneDistSampler, transform_val, img_insert_trigger, format_prompt, merge_data
...
def get_args_parser():
    ...
    parser.add_argument('--use_acc', action="store_true", default=False,
                        help="Whether to use the test accuracy to measure the performance of the target model")
...
def evaluate(model, dataset, transform, step_size, trig_pos, trig_size, device, target_output, max_gen_len=1024, gen_t=0.6, top_p=0.9, use_acc=False):
    model.eval()
    ...
    for i in range(len(indices) // step_size):
        if temp_dataset.prefix:
            filenames = [temp_dataset.img_path + "/{}_{}".format(temp_dataset.prefix, temp_dataset.ann[idx]["image"]) for idx \
                in indices[i*step_size:(i+1)*step_size]]
        else:
            filenames = [temp_dataset.img_path + "/{}".format(temp_dataset.ann[idx]["image"]) for idx \
                in indices[i*step_size:(i+1)*step_size]]
        image_backdoor_indices = [temp_dataset.img_backdoor_indices[idx] for idx in indices[i*step_size:(i+1)*step_size]]
        questions = [format_prompt(temp_dataset.ann[idx]['conversations'][0]['value']) for idx in indices[i*step_size:(i+1)*step_size]]
        ...
        for j in range(len(results)):
            ...
            print("#{}\nQuestion:\n{}\nImage:\n{} (backdoor: {})\nAnswer:\n{}\nResult:\n{}".format(curr_idx, temp_dataset.ann[indices[curr_idx]]['conversations'][0]['value'],\
            temp_dataset.ann[indices[curr_idx]]["image"], image_backdoor_indices[j], temp_dataset.ann[indices[curr_idx]]['conversations'][1]['value'], results[j]))

            if target_output.lower() in results[j].lower():
                attack_success += 1
            
            if use_acc and results[j].lower().startswith(temp_dataset.ann[indices[curr_idx]]['conversations'][1]['value'].lower()):
                predict_success += 1
    if use_acc:
...
def main(args):
    ...
    if args.target_data:
        ...
        all_data = ['clean', 'backdoor', 'neg_img', 'neg_instruct']

    for data_name in all_data:
        _time = time.time()
        print('*'*50)
        print("Test data type: {}".format(data_name))
        print('*'*50)
        ...
        for idx in range(args.n_eval):
            if data_name == 'clean':
                ...
                _train_poison_dataset, data = original_data.get_backdoor_data(train_dataset, test_dataset, poison_ratio=args.poison_ratio, target_output=args.target_output, \
                    attack_type=args.attack_type, trig_text=args.trig_text, trig_pos=trig_pos, trig_size=eval(args.trig_size), seed = args.seed)
            elif data_name == 'neg_img':
                data = test_neg_img_data
            elif data_name == 'neg_instruct':
            ...
            result = evaluate(model, data, transform_val, step_size=args.step_size, trig_pos=trig_pos[0], trig_size=eval(args.trig_size), device=device, target_output=args.target_output, use_acc=args.use_acc)
            if args.use_acc:
                print("({}) | ASR: {:.2f}%, Test accuracy: {:.2f}% | Data type: {}".format(idx, 100 * result[0], 100 * result[1], data_name))
                avg_acc += result[1]
            else:
                print("({}) | ASR: {:.2f}% | Data type: {}".format(idx, 100 * result[0], data_name))
            avg_asr += result[0]
        ...
        print('-'*30)
        if args.use_acc:
            print("Average ASR: {:.2f}%, Test accuracy: {:.2f}% | Data type: {}".format(100 * avg_asr, 100 * avg_acc, data_name))
        else:
            print("Average ASR: {:.2f}% | Data type: {}".format(100 * avg_asr, data_name))
