...
from data.dataset_llava import FinetuneDataset, data_transform, img_insert_trigger
...
def get_args_parser():
    ...
    parser.add_argument('--trig_pos', nargs='?', default='cc|random',  
...
def evaluate(model, dataset, transform, step_size, trig_pos, trig_size, device, target_output, max_gen_len=1024, use_acc=False):
    ...
    for i in range(len(indices) // step_size):
        ...
        questions = [llama.format_prompt(temp_dataset.ann[idx]['conversations'][0]['value']) for idx in indices[i*step_size:(i+1)*step_size]]
...
def main(args):
    ...
    for data_name in all_data:
        ...
        for idx in range(args.n_eval):
            ...
            result = evaluate(model, data, data_transform, step_size=args.batch_size, trig_pos=trig_pos[0], trig_size=eval(args.trig_size), device=device, target_output=args.target_output, max_gen_len=args.max_words, use_acc=args.use_acc)
            if args.use_acc:
        ...
        print('-'*30)
        if args.use_acc:
            print("Average ASR: {:.2f}%, Test accuracy: {:.2f}% | Data type: {}".format(100 * asr / args.n_eval, 100 * clean_acc / args.n_eval, data_name))
        else:
            print("Average ASR: {:.2f}% | Data type: {}".format(100 * asr / args.n_eval, data_name))
